> **[ARCHIVED] 归档说明 (Archived on: 2026-04-05)**
> 
> 本文档记录了本平台早期（第一/第二阶段）的架构重构与踩坑历史。其中的全部任务均已 `[✅ 已完成]`。
> 
> ⛔ **AI Agent 警告**：
> 请勿根据本文档执行任何代码重构任务！本文档仅作为历史参考保留。
> 关于当前的系统架构规划、最新需求以及您的任务追踪列表，**请必须以项目根目录下的 `PROJECT_ANALYSIS.md` 为唯一准则。**
...
### 7.4. [✅ 已完成] 性能优化专录：消除 TCP 链路中的累积延迟 (TCP Buffer Bloat Fix)
...
### 7.5. [✅ 已完成] 修正隐藏逻辑漏洞：清扫 TCP 优化后暴露的“自动驾驶”缺陷 (Fix Hidden Logical Flaws)

**业务痛点：**
在完全消除了 7.4 描述的 TCP 堆积和粘包丢帧现象后，数据流变得极度顺畅，但机器人却出现了“在未使用且未集中注意力时，自动以恒定速度向前运行”的严重异常。

**问题根因 (Root Cause Analysis)：**
随着 TCP 层的阻塞被疏通，`read_frame()` 不再高频拉响长达 0.5 秒报错/超时的看门狗（Watchdog），于是看门狗强制停车的“遮羞布”被揭开，暴露出原版本身自带的两个致命隐患：
1. **FocusPolicy 颠倒的映射逻辑**：专注度计算核心错误地引入了一层反转（`speed_intent = 1.0 - focus_norm`）。这使得穿戴者处于“无脑/低专注基线（ `focus_norm` ~ 0.0）”时，管线输出的 `speed_intent` 常态化锁定在极值 `1.0`（满速前行）。
2. **轮询空转期 (Empty-tick) 的硬编码污染**：上游 EEG 设备帧率（比如 10Hz）与下游 ROS 定时器消费频率（20Hz）的不匹配必定会产生空缺轮次。原 `eeg_control_node.py` 的硬代码由于缺乏状态缓存，在处理包含有效的 `movement` 更新后，错误地遗留下了一枚 `{"speed_intent": 0.5}` 的状态毒药。这导致系统在紧随其后的“空缺轮次”中，错误地将这枚毒药代入 fallback 环节转换为前进速度并发送给机器引擎。

**架构级解决方案 (Architectural Fix Plan)：**
1. 行动：剥离 `FocusPolicy` 中的倒置公式。使控制意图的逻辑完全与人类直觉贴合，归一化基线归 0。
2. 行动：为 `eeg_control_node.py` 下的空缺轮次回退（Fallback）注入了 `last_mode`（最后工作模式）与 `last_twist` （最后扭矩映射）的防串扰墙。在 Movement 模式遇到空帧时，完美回放其本身的运动指令，彻底切断了与 `last_intents` 默认常量的纠缠。
