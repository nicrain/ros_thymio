...
当前 `lsl_test/` 的 DSP 管线从原始信号直接进入 Welch PSD 计算，中间没有任何数据清洗或滤波步骤。Phase 1 管线验证阶段是可接受的。

### 7.5.1 当前状态与约束

- **g.tec 设备尚未到位**，无法评估其信号特性（是否自带滤波、输出单位、噪声水平等）。
- 当前仅有 Enobio 设备的 EDF 文件作为参考数据。
- 滤波实现应**推迟到 g.tec 到位后**，根据实际信号质量评估结果决定需要哪些步骤。
- 如需验证滤波效果，可先用 Enobio EDF 文件进行实验性对比。

### 7.5.2 潜在的预处理步骤

以下步骤根据优先级和设备特性决定是否实现：

| 步骤 | 作用 | 影响的频段 | 实现时机 |
|------|------|-----------|---------|
| 带通滤波 (Bandpass) | 去除 DC 漂移 (<0.5Hz) 和高频噪声 (>45Hz) | delta, gamma | g.tec 到位后评估 |
| 陷波滤波 (Notch) | 去除工频干扰 (50Hz 或 60Hz) | beta, gamma | 仅当带通 >45Hz 时需要 |
| 伪迹去除 (Artifact Rejection) | 去除眼电 (EOG)、肌电 (EMG) 等伪迹 | delta, theta, 全频段 | 视信号质量决定 |
| 重参考 (Re-referencing) | 公共平均参考 (CAR) 等 | 全频段 | 视设备配置决定 |

**关于陷波滤波：** 如果带通截止频率 ≤45Hz（当前默认值），50Hz 工频干扰已被带通滤除，陷波滤波器冗余。仅当需要保留 gamma 频段（带通扩展到 100Hz）时才需要单独的陷波滤波。

### 7.5.3 设计约束

1. 滤波必须在 Welch PSD 计算之前执行，作为 DSP 管线的可选前置步骤。
2. 滤波参数（截止频率、滤波器阶数等）必须可通过 `DSPConfig` 或 `experiment_config.yaml` 配置。
3. 滤波步骤可通过配置开关启用/禁用，便于对比滤波前后的效果。
4. 带通滤波建议使用 Butterworth 或 Chebyshev IIR 滤波器（低延迟），或 FIR 滤波器（线性相位）。
...
### 7.5.5 滤波参数默认值（预留，g.tec 到位后根据实际情况调整）

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `bandpass_low` | 0.5 Hz | 高通截止频率 |
| `bandpass_high` | 45.0 Hz | 低通截止频率 |
| `bandpass_order` | 4 | 滤波器阶数 |
| `notch_freq` | 50.0 Hz | 陷波中心频率（欧洲/中国） |
| `notch_quality` | 30.0 | 陷波品质因数 Q |
| `enable_bandpass` | False | 带通滤波开关（默认关闭） |
| `enable_notch` | False | 陷波滤波开关（默认关闭） |

**默认关闭的原因：** g.tec 设备的信号特性未知，滤波可能不必要甚至有害（如设备已自带滤波）。待设备到位、信号质量评估完成后，再根据实际情况启用和调参。

## 7.6 流处理边角情况与性能优化路线（v2.1 新增）
...
2. 原 §7.5-7.8 重编号为 §7.6-7.9。
3. §7.5 更新：g.tec 设备尚未到位，滤波需求改为前瞻性文档，默认开关关闭，待设备到位后评估再启用。
