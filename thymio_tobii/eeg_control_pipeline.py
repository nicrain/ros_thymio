...
class KeyboardAdapter(BaseAdapter):
    ...
    def read_frame(self) -> Optional[EegFrame]:
        # 注意：由于在 ROS2 Node 中运行，真正的键盘捕获通常需要 termios
        # 这里为了“最小可运行版本”，我们让它返回当前的模拟状态
        # 实际交互逻辑建议在 Node 层通过命令行或 Mock 逻辑快速模拟
        return EegFrame(
            ts=time.time(),
            source="keyboard",
...
def main() -> int:
    ...
    try:
        ...
        print(f"ERROR: cannot initialize adapter: {e}")
        return 1
    ...
    print(
        f"[pipeline] input={args.input} policy={args.policy} -> udp {args.udp_host}:{args.udp_port} at {args.hz:.1f}Hz"
    )

    last_emit = 0.0
    last_intents = {"speed_intent": 0.5, "steer_intent": 0.5}
    ...
    try:
        while True:
            ...
            if frame is not None:
                ...
                last_intents = policy.compute_intents(features)
                if args.verbose:
                    print(
                        "[pipeline] src=%s alpha=%.3f theta=%.3f beta=%.3f t/b=%.3f b/(a+t)=%.3f speed_intent=%.3f steer_intent=%.3f"
                        % (
                            frame.source,
                            ...
                            last_intents["speed_intent"],
                            last_intents["steer_intent"],
                        )
                    )

            if now - last_emit >= dt:
                payload_obj = dict(last_intents)
                if not args.disable_legacy_xy:
                    payload_obj = with_legacy_xy(payload_obj)
